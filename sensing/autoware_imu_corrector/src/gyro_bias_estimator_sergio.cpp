...
#include "gyro_bias_estimator_sergio.hpp"
...
{
GyroBiasEstimatorSergio::GyroBiasEstimatorSergio(const rclcpp::NodeOptions & options)
: rclcpp::Node("gyro_bias_scale_validator_sergio", options),
  ...
  gyro_bias_(std::nullopt)
{
  ...
  updater_.add("gyro_bias_scale_validator", this, &GyroBiasEstimatorSergio::update_diagnostics);
  ...
  auto bound_timer_callback = std::bind(&GyroBiasEstimatorSergio::timer_callback, this);
...
void GyroBiasEstimatorSergio::callback_imu(const Imu::ConstSharedPtr imu_msg_ptr)
{
...
void GyroBiasEstimatorSergio::callback_odom(const Odometry::ConstSharedPtr odom_msg_ptr)
{
...
void GyroBiasEstimatorSergio::callback_pose_msg(
  const PoseWithCovarianceStamped::ConstSharedPtr pose_msg_ptr)
{
  estimate_scale_gyro(pose_msg_ptr);
}

void GyroBiasEstimatorSergio::estimate_scale_gyro(
  const PoseWithCovarianceStamped::ConstSharedPtr pose_msg_ptr)
{
  ...
  if (gyro_bias_.has_value() && gyro_buf_.size() > static_cast<size_t>(buffer_size_gyro_ - 2)) {
    ...
    if (rate_pose_buff_.size() > static_cast<size_t>(samples_filter_pose_rate_)) {
      avg_rate_pose_ = std::accumulate(rate_pose_buff_.begin(), rate_pose_buff_.end(), 0.0) /
      ...
      y_ =
        (avg_rate_gyro_ - gyro_bias_not_rotated_.value().z) - (estimated_scale_ * avg_rate_pose_);
      s_ = h_ * p_ * h_ + r_;
      k_ = p_ * h_ / s_;
...
void GyroBiasEstimatorSergio::timer_callback()
{
  if (pose_buf_.empty()) {
...
void GyroBiasEstimatorSergio::validate_gyro_bias()
{
...
geometry_msgs::msg::Vector3 GyroBiasEstimatorSergio::transform_vector3(
...
void GyroBiasEstimatorSergio::update_diagnostics(diagnostic_updater::DiagnosticStatusWrapper & stat)
{
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::imu_corrector::GyroBiasEstimatorSergio)
