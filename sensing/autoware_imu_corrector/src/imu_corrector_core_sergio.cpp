...
#include "imu_corrector_core_sergio.hpp"
...
std::array<double, 9> transform_covariance_sergio(const std::array<double, 9> & cov)
{
...
geometry_msgs::msg::Vector3 transform_vector3_sergio(
...
{
ImuCorrectorSergio::ImuCorrectorSergio(const rclcpp::NodeOptions & options)
: rclcpp::Node("imu_corrector_sergio", options),
  output_frame_(declare_parameter<std::string>("base_link", "base_link"))
{
  ...
  imu_sub_ = create_subscription<sensor_msgs::msg::Imu>(
    "input", rclcpp::QoS{1}, std::bind(&ImuCorrectorSergio::callback_imu, this, std::placeholders::_1));
  gyro_bias_sub_ = create_subscription<Vector3Stamped>(
    "gyro_bias_input", rclcpp::SensorDataQoS(),
    std::bind(&ImuCorrectorSergio::callback_bias, this, std::placeholders::_1));
  gyro_scale_sub_ = create_subscription<Vector3Stamped>(
    "gyro_scale_input", rclcpp::SensorDataQoS(),
    std::bind(&ImuCorrectorSergio::callback_scale, this, std::placeholders::_1));
  RCLCPP_INFO(this->get_logger(), "Created subscriptions for Sergio version");
...
void ImuCorrectorSergio::callback_imu(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg_ptr)
{
  ...
  imu_msg_base_link.linear_acceleration =
    transform_vector3_sergio(imu_msg.linear_acceleration, *tf_imu2base_ptr);
  imu_msg_base_link.linear_acceleration_covariance =
    transform_covariance_sergio(imu_msg.linear_acceleration_covariance);
  imu_msg_base_link.angular_velocity =
    transform_vector3_sergio(imu_msg.angular_velocity, *tf_imu2base_ptr);
  imu_msg_base_link.angular_velocity_covariance =
    transform_covariance_sergio(imu_msg.angular_velocity_covariance);

  imu_pub_->publish(imu_msg_base_link);
}

void ImuCorrectorSergio::callback_bias(const Vector3Stamped::ConstSharedPtr bias_msg_ptr)
{
  // update gyro bias
  gyro_bias_ = *bias_msg_ptr;
}

void ImuCorrectorSergio::callback_scale(const Vector3Stamped::ConstSharedPtr scale_msg_ptr)
{
  // update gyro scale
...
RCLCPP_COMPONENTS_REGISTER_NODE(autoware::imu_corrector::ImuCorrectorSergio)